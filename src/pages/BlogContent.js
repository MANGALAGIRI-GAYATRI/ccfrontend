...
const BlogContent = () => {
  ...
  useEffect(() => {
    const getBlog = async () => {
      try {
        const { data } = await axios.get(`https://cc-backend-ckz1.onrender.com/api/v1/blog/get-blog/${id}`);
        if (data?.success) {
          setBlog(data?.blog);
  ...
  const handleSubmitComment = async () => {
    try {
      ...
      const response = await axios.post(`https://cc-backend-ckz1.onrender.com/api/v1/blog/add-comment/${id}`, commentData);
